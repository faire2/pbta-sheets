...
import { extractAll } from "./generate-skin-messages"
  ...
  const walk = (
    ...
    if (Array.isArray(enNode)) {
      if (!Array.isArray(csNode)) {
        fail(`${skinId}: ${path} should be a list`)
        return
      }
      if (enNode.length !== csNode.length) {
        fail(
          `${skinId}: ${path} has ${String(csNode.length)} items, English has ${String(enNode.length)} — positional, so it would silently fall back`,
        )
      }
      return
    }
    if (typeof enNode === "object" && enNode !== null) {
      if (typeof csNode !== "object" || csNode === null || Array.isArray(csNode)) {
        fail(`${skinId}: ${path} should be an object`)
        return
      }
      ...
      for (const key of Object.keys(csObj)) {
        if (!(key in enObj)) {
          fail(`${skinId}: ${path}.${key} is not in the English catalogue`)
          continue
        }
        walk(enObj[key], csObj[key], `${path}.${key}`, skinId)
      }
      return
    }
    if (typeof enNode === "string" && typeof csNode !== "string") {
      fail(`${skinId}: ${path} should be a string`)
    }
  }
