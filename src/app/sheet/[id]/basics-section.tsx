...
import {
  localizeBasicMoves,
  localizeRulesEntries,
} from "@/data/localize-rules"
        ...
        <ul className="border-rule mt-4 border-t">
          {moves.map((move) => (
            <li key={move.id} className="border-rule border-b py-4">
              ...
              <p className="text-ink-soft mt-1.5 font-sans text-[0.93rem] leading-snug italic">
                {move.trigger}
              </p>
              <dl className="mt-2.5 space-y-1.5">
                <div className="flex gap-2.5">
                  <dt className="text-ink-faint w-9 shrink-0 pt-0.5 font-sans text-[0.7rem] tracking-[0.1em] tabular-nums">
                    {t("sheet.strongHit")}
                  </dt>
                  <dd className="text-ink flex-1 font-sans text-[0.93rem] leading-snug">
                    {move.strongHit}
                  </dd>
                </div>
                <div className="flex gap-2.5">
              ...
              {move.options.length > 0 ? (
                <ul className="border-rule mt-2.5 space-y-1 border-l pl-3">
                  {move.options.map((option) => (
                    <li
                      key={option}
                      className="text-ink-soft font-sans text-[0.9rem] leading-snug"
                    >
                      {option}
                    </li>
                  ))}
        ...
        <ul className="border-rule mt-4 border-t">
          {rules.map((entry) => (
            <li key={entry.id} className="border-rule border-b py-4">
              <h3 className="font-display text-ink text-[1.15rem] leading-tight tracking-wide">
                {entry.name}
              </h3>
              <p className="text-ink-soft mt-1.5 font-sans text-[0.93rem] leading-snug">
                {entry.summary}
              </p>
