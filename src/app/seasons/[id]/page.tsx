...
export default async function SeasonPage({ params }: PageProps<"/seasons/[id]">) {
  ...
  const roster = await db.select().from(characters).where(eq(characters.seasonId, season.id))
  ...
  return (
    <>
      ...
      <main className="mx-auto w-full max-w-2xl flex-1 px-5 pt-8 pb-20 sm:px-8">
        ...
        {canEdit ? (
          <SeasonEditor id={season.id} name={season.name} joinCode={season.joinCode} />
        ) : (
          <header className="mt-5">
            ...
            <p className="text-ink-soft mt-3 font-sans text-[1.044rem]">
              <span className="font-mono tracking-[0.24em]">{season.joinCode}</span>
            </p>
              ...
              {roster.map((character) => {
                ...
                return (
                  <li key={character.id} className="border-rule border-b">
                    ...
                    >
                      <span className="font-display text-ink flex-1 text-[1.45rem] leading-none tracking-wide">
                        {character.name || t("common.unnamed")}
                      </span>
                      <span className="text-ink-soft font-sans text-[1.044rem] italic">
